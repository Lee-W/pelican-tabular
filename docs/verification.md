...
## 2026-09-21：地點參照索引修正（尚未發布）

全域參照掃描略過 `_schema.yaml`、`_common.yml` 等底線開頭的檔案，
並支援以 ID 為 key 的地點資料。三種地點格式都會保留預設值、個別覆寫、
標籤合併及巢狀 `items`；沒有預設標籤清單時，一般資料的 `tags` 保持原值。
有非空的預設標籤清單時，空值與假值仍繼承預設標籤；非空的非清單值則直接覆寫。
明確指定檔案的參照仍可載入底線開頭的資料檔，損壞的資料仍會回報錯誤。

- `uv run pytest -n 0 -q`：264 項通過。
- `uv run poe lint` 與修改檔案的 Ruff format 檢查通過。
- entertainment-blog 實際參照索引：118 個 ID、272 個名稱，含 30 筆影廳資料；
  所有現有全域 ID 參照皆可唯一解析，沒有索引警告。
- 透過暫時的 `uv run --with-editable` 環境載入本機套件並核對來源檔內容，
  完整產站與 Pagefind 成功，沒有 Pelican 警告或錯誤。
- 中日文排名表格 HTML 與修正前相同，各保留 712 筆作品；演唱會、影廳與
  日文隨機入口正常產生。

部落格的 pyproject、lockfile 與原本 `.venv` 仍使用已發布的 0.8.0。
以上是本機修正版的驗證結果；須發布新版後再正式升級部落格。

### 0.8.1 發布前複核

- `uv run --locked poe lint`、`uv run --locked poe cover`：Ruff／mypy 通過，
  264 項測試通過，覆蓋率 91%。`npm test` 的 4 項測試通過。
- 本次變更檔案的 `prek` hooks 全部通過；未執行全庫 `poe ci`。
- `uv build` 成功產生 wheel／sdist。版本依發布流程保留 0.8.0，
  `uv run cz bump --get-next --yes` 確認合併後預期為 0.8.1。
- 在隔離副本移除參照修正，13 項新增測試有 9 項失敗；以解壓後的
  wheel 載入相同測試，13 項全過，且已核對 import 來自該 wheel。
- 用該 wheel 產生範例的兩個頁面，確認資料庫 HTML、CSS 和完整的
  core／views JS bundle。另驗證 27 組標籤預設值案例、清單中途切換
  defaults 的行為，以及文件的 14 個本機檔案連結。
- 這次複核未重跑瀏覽器測試與兩站完整產站；前述整合紀錄仍保留其各自的驗證範圍。

## 2026-09-20：將部落格試用版介面回寫套件（已隨 0.8.0 發布）
